...
- **`blocked` (`reason: "report_verification_failed"`)** — the verifier failed twice. Surface the returned report paths; keep the research files. Verify those exact files before any synthesis-only recovery. Do not claim recommendations or complete coverage.
