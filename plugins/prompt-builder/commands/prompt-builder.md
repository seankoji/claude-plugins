...
Use the operator's target model when specified; otherwise use the current session model. Record the resolved model and settings used in the evaluation. Do not pin a release number from this command or claim a cheaper model is adequate without testing it.
