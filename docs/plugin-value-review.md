...
| ape | Conditional. Source comparisons are valuable when tied to a local gap. Recommendations are hypotheses until an adoption experiment succeeds. | Synthesize only the current run's explicit reports; failed analysts are excluded and listed in partial results; missing files block completion. A verifier outage is retried, then explicitly labelled unverified. Allow zero recommendations and require a simpler alternative, baseline, pass condition, and abandon condition. | The source is already known, so use `study`, or there is no concrete local weakness worth researching. |
