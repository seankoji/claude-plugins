...
| ape | Conditional. Source comparisons are valuable when tied to a local gap. Recommendations are hypotheses until an adoption experiment succeeds. | Refresh the fingerprint and allocate a fresh report directory each run. Synthesize only successful analysts' reports, listing partial coverage; retry verification once, then stop with the report paths preserved if verification fails. Allow zero recommendations and require a simpler alternative, baseline, pass condition, and abandon condition. | The source is already known, so use `study`, or there is no concrete local weakness worth researching. |
...
available. The bundled comparator only regression-checks transcribed outputs across runtimes; it does
...
The local CLI's `claude plugin details imps@seankoji` estimated about 39,000 tokens
on invocation for installed imps 0.3.56. This is projected instruction overhead, not
measured model billing. The workflow should earn that cost on substantial tasks.
